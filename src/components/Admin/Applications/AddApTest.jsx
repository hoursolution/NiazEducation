...
const AddApplicationForm = () => {
  ...
  const getFieldsForTab = (tabIndex) => {
    switch (tabIndex) {
      ...
      case 5:
        return []; // Degree form handled separately
      case 6:
        return [
          "disabled_parent_cnic_doc",
          "b_form_doc",
  ...
  const handleContinue = () => {
    ...
    fieldsForTab.forEach((fieldName) => {
      ...
      if (error) tabErrors[fieldName] = error;
    });

    if (activeTab === 5) {
      const degreeError = validateDegreeForm();
      if (degreeError)
        return setAlert({ severity: "error", message: degreeError });
    }
      ...
      setFormData((prev) => {
        ...
        if (name === "date_of_birth" && !error && value) {
          ...
          if (!isNaN(selectedDate.getTime())) {
            ...
            newData.age = ageInYears.toString();
          }
        }
  ...
  const removeDegree = (index) => {
    setDegreeForm((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    documentFields.forEach((field) => {
      if (["disabled_parent_cnic_doc", "b_form_doc"].includes(field)) {
        if (formData[field].length === 0)
          documentErrors[field] = "Please upload at least one file";
      } else {
        if (!formData[field]) documentErrors[field] = "Please upload a file";
      }
    });
    ...
    if (degreeFormError) {
      setLoading(false);
      setAlert({ severity: "error", message: degreeFormError });
      return;
    }
    ...
    ["disabled_parent_cnic_doc", "b_form_doc"].forEach((field) => {
      formData[field].forEach((file) => formDataObject.append(field, file));
    });
    ...
    [
      ...
      if (formData[field]) formDataObject.append(field, formData[field]);
    });
    ...
    try {
      ...
      const updatedDegreePromises = degreeForm.map(async (degree, index) => {
        if (degree.isNew) {
          ...
          // Optionally replace the new entry with returned one (which will have an id)
          setDegreeForm((prev) => {
            const updated = [...prev];
          ...
          console.warn("Skipping invalid degree without ID:", degree);
          return Promise.resolve();
        }
      });

      await Promise.all(updatedDegreePromises);

      setAlert({
        severity: "success",
        message: "Application updated successfully!",
      });
      setTimeout(() => navigate("/admin/allApplications"), 2000);
    } catch (error) {
    ...
    <StyledContainer maxWidth="lg">
      <FormPaper elevation={0}>
        ...
        >
          ...
          <StyledTab label="Education Detail" />
            ...
            >
              <div>
                {activeTab === 5 && (
                  <CardAnimation>
                    <Paper sx={{ padding: 3, borderRadius: 3, mb: 3 }}>
                      <SectionTitle variant="h6">Education Detail</SectionTitle>
